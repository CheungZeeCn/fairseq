...
class IterativeRefinementGeneratorRbtp(object):
    def __init__(
        self,
        tgt_dict,
        models=None,
    ...
    def generate(self, models, sample, prefix_tokens=None, constraints=None):
        ...
        bsz, src_len = src_tokens['input_ids'].size()

        # initialize
        #encoder_out = model.forward_encoder([src_tokens, src_lengths])
        encoder_out = model.forward_encoder([src_tokens])
        #encoder_out = model.forward_encoder(src_tokens)
        #logging.info(encoder_out)
        #raise NotImplementedError()

        # 这块已经更改为
        prev_decoder_out = model.initialize_output_tokens(encoder_out, src_tokens)
        # logging.info("before prev_decoder_out {}".format(prev_decoder_out))

        # 这种支持需要额外开发，lev-t 默认不支持，所以先忽略就好了
        if self.beam_size > 1:
            ...
            # [0,0,0,1,1,1,2,2,2,3,3,3,4,4,4,5,5,5] # 假设 beam_size=3, bsz=5
            length_beam_order = utils.new_arange(src_tokens, self.beam_size, bsz).t().reshape(-1)
            # 核心就是使用 index_select 来重新组装和排序 encoder_out 的内容
            encoder_out = model.encoder.reorder_encoder_out(encoder_out, length_beam_order)
            # todo 这块就是没有很看懂, 重新梳理的decoder 的输出？
            #  针对每个输入，decoder的输出应该是不一样的,  应该做一些分组排序的动作？
            # 再跟进了一下，这块不是普通的beam search， 这块是length的beam search
            prev_decoder_out = model.regenerate_length_beam(prev_decoder_out, self.beam_size)
            #logging.info("prev_decoder_out {}".format(prev_decoder_out))
            #raise NotImplementedError("beam_size")
        ...
        for step in range(self.max_iter + 1):
            ...
            if self.adaptive: # early stop， 发现生成出loop了
        ...
        # logging.info("finalized: {}".format(finalized))
        return finalized
